...
const DISCORD_TIP_DISMISS_KEY = "anikura:watch-discord-tip-dismissed";
...
export function WatchExperience(props: Props) {
  ...
  const [discordTipVisible, setDiscordTipVisible] = useState(true);
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem(DISCORD_TIP_DISMISS_KEY) === "1") {
        setDiscordTipVisible(false);
      }
    } catch {
      /* ignore */
    }
  ...
  function dismissDiscordTip() {
    setDiscordTipVisible(false);
    try {
      sessionStorage.setItem(DISCORD_TIP_DISMISS_KEY, "1");
    } catch {
      /* ignore */
    }
  }
          ...
          <div className="page-shell pt-4">
            ...
            <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px]">
              <div className="min-w-0">
                ...
                {discordTipVisible && (
                  <div
                    className={`${tipVisible ? "mt-2" : "mt-3"} flex items-start gap-3 rounded-xl border border-white/10 bg-raised/70 px-3.5 py-2.5 text-sm text-cloud`}
                  >
                    <DiscordTipIcon />
                    <p className="min-w-0 flex-1 leading-snug">
                      Get site updates — feedback &amp; bugs welcome.{" "}
                      <a
                        href={ANIKURA_DISCORD_INVITE}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-medium text-snow underline decoration-white/25 underline-offset-2 transition hover:decoration-white/60"
                      >
                        Join Discord →
                      </a>
                    </p>
                    <button
                      type="button"
                      aria-label="Dismiss Discord tip"
                      onClick={dismissDiscordTip}
                      className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-mute transition hover:bg-white/10 hover:text-snow"
                    >
                      ×
                    </button>

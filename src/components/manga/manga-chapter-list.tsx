...
import { mangaReadHref } from "@/lib/atsu";
import type { MangaChapter } from "@/lib/manga-types";
...
export function MangaChapterList({ mangaId, chapters }: Props) {
    ...
    <section className="mt-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="sr-only" htmlFor="manga-chapter-search">
            Search chapters
          </label>
          <input
        ...
        <>
          <ul className="mt-6 divide-y divide-white/[0.06] overflow-hidden rounded-[1.25rem] border border-white/[0.08] bg-white/[0.02]">
            {visible.map((chapter) => (
              <li key={chapter.id}>
                <Link
                  href={mangaReadHref(mangaId, chapter.id)}

-- Track session start so Night desk can show real session duration.

alter table public.presence
  add column if not exists first_seen timestamptz;

-- Best-effort backfill: existing rows only had last_seen.
...
begin
  ...
  insert into public.presence as pr (
    session_id, user_id, lat, lng, country, city, path, first_seen, last_seen
  )
  values (
